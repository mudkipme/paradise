import { ChainLink, Item, Location, Nature } from "pokedex-promise-v2";
...
export interface IEvolveOptions {
    location?: Location;
...
export async function findEvolution(pokemon: Pokemon, trigger: string, options: IEvolveOptions) {
    const [species, holdItem, trainer] = await Promise.all([pokemon.species(),
        pokemon.holdItem(), pokemon.getTrainer()]);
    // Pokémon can't evolve when holding an Everstone
    if (holdItem && holdItem.name === "everstone") {
        return;
    }
    const [evolvesTo, stats] = await Promise.all([species.evolvesTo(), pokemon.stats()]);
    ...
    for (const chain of evolvesTo) {
        ...
        for (const evolutionDetail of chain.evolution_details) {
            ...
            if (evolutionDetail.party_species) {
                const speciesList = await speciesListOfTrainerParty(trainer);
                const found = speciesList.some((entry) =>
                    entry.pokemonSpecies.name === evolutionDetail.party_species!.name);
                if (!found) {
                    continue;
                }
            }
            if (evolutionDetail.gender && evolutionDetail.gender.name !== pokemon.gender) {
                continue;
            }
            if (evolutionDetail.trade_species) {
                ...
                const tradePokemonSpecies = await options.tradePokemon.species();
    ...
    return Species.find(pokemonSpecies.id, pokemon.formIdentifier);
}

async function speciesListOfTrainerParty(trainer: Trainer | null) {
    if (!trainer) {
        return [];
    }
